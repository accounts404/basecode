    ...
    try {
            ...
            for (const cleanerId of schedule.cleaner_ids) {
                ...
                if (cleanerRate === 0) {
                    ...
                    calculatedEntries.push({
                        ...
                        cleaner_name: cleanerUser?.invoice_name || cleanerUser?.full_name || 'Desconocido',
                        error: 'NO_RATE',
                        error_message: 'El limpiador no tiene tarifa configurada para esta fecha. Verifica el rate_history en el perfil del limpiador.',
                        service_date: format(workDate, 'yyyy-MM-dd')
                    });
                    continue;
                }
        ...
        if (mode === 'preview') {
            ...
            for (const entryData of entriesToCreate) {
                // Si tiene error (ej: NO_RATE), agregar a skipped y continuar
                if (entryData.error) {
                    console.log(`\n>>> ⚠️ Error en entrada para: ${entryData.cleaner_name} - ${entryData.error_message}`);
                    skippedEntries.push({
                        cleaner_id: entryData.cleaner_id,
                        cleaner_name: entryData.cleaner_name,
                        reason: entryData.error_message,
                        service_date: entryData.service_date
                    });
                    continue;
                }
                ...
                if (finalCheck.length > 0) {
                    ...
                    skippedEntries.push({
                        cleaner_id: entryData.cleaner_id,
                        cleaner_name: entryData.cleaner_name,
                        reason: 'Ya existe WorkEntry para este servicio',
