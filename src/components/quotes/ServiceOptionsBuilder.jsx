        ...
        <div className="grid grid-cols-3 gap-4 h-[70vh]">
          ...
          <div className="flex flex-col gap-3 border-r pr-4">
            <div className="flex justify-between items-center">
              <h3 className="font-semibold text-sm">Opciones Creadas ({options.length})</h3>
            ...
            <ScrollArea className="flex-1">
              {options.length === 0 ? (
                <div className="text-center text-muted-foreground py-8">
                  ...
                  <p className="text-sm">No hay opciones</p>
                </div>
              ) : (
                      ...
                      <CardHeader className="p-3">
                        <div className="flex justify-between items-start gap-2">
                          <div className="flex-1 min-w-0">
                            <CardTitle className="text-sm truncate">{option.option_name}</CardTitle>
                            <p className="text-xs text-muted-foreground mt-1">
                              {option.selected_areas_items?.reduce((acc, area) => acc + (area.selected_items?.length || 0), 0) || 0} items
                            </p>
                          </div>
                          <Button 
                            variant="ghost" 
          ...
          <div className="flex flex-col gap-3 border-r pr-4 overflow-y-auto">
            {editingOption ? (
              <div className="space-y-3">
                <div>
                  <Label className="text-sm">Nombre de la Opción</Label>
                  <Input
                    value={editingOption.option_name}
                    onChange={(e) => setEditingOption({ ...editingOption, option_name: e.target.value })}
                    placeholder="Ej: Full Home Cleaning"
                  />
                </div>
                      ...
                      {Object.keys(frequencyLabels).map(freq => {
                        ...
                        return (
                          <div key={freq} className="flex items-center space-x-2 p-2 border rounded-lg hover:bg-gray-50">
                            ...
                            />
                            <Label htmlFor={`freq-${freq}`} className="cursor-pointer text-xs font-medium">
                    ...
                    <div className="space-y-2 pr-3">
                      {serviceType === 'initial' ? (
                        <div className="border rounded-lg p-2">
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            <div>
                              <Label className="text-xs text-muted-foreground">Min (AUD)</Label>
                              <Input
                                type="number"
                            ...
                            <div>
                              <Label className="text-xs text-muted-foreground">Max (AUD)</Label>
                              <Input
                                type="number"
                ...
                <Button onClick={saveOption} className="w-full" size="sm">
              ...
              <div className="flex-1 flex items-center justify-center text-muted-foreground">
                <div className="text-center">
                  ...
                  <p className="text-sm">Selecciona o crea una opción</p>
                </div>
              </div>
            )}
          </div>

          {/* Right: Area & Items Selection */}
          <div className="flex flex-col gap-3 overflow-hidden">
            {editingOption ? (
              <>
                <Label className="text-sm">Selecciona Áreas e Items para Incluir</Label>
                <Tabs value={activeArea} onValueChange={setActiveArea} className="flex-1 flex flex-col overflow-hidden">
                  <TabsList className="grid w-full grid-cols-3">
                    {areas.slice(0, 6).map(area => (
                      <TabsTrigger key={area.id} value={area.id} className="text-xs px-1">
                        {area.name.split(' ')[0]}
                      </TabsTrigger>
                    ))}
                  </TabsList>
                  ...
                  {areas.map(area => {
                    ...
                    return (
                      <TabsContent key={area.id} value={area.id} className="flex-1 overflow-y-auto mt-2">
                        <div className="space-y-2">
                          ...
                          >
                            <div className="flex items-center space-x-2 p-2 border rounded-lg hover:bg-gray-50">
                              ...
                              <Label htmlFor={`${area.id}-full`} className="flex-1 cursor-pointer">
                                <div className="font-semibold text-xs">Servicio Completo</div>
                                <div className="text-xs text-gray-500">Todos los items</div>
                              </Label>
                            ...
                            <div className="flex items-center space-x-2 p-2 border rounded-lg hover:bg-gray-50">
                              ...
                              <Label htmlFor={`${area.id}-custom`} className="flex-1 cursor-pointer">
                                <div className="font-semibold text-xs">Personalizado</div>
                                <div className="text-xs text-gray-500">Items específicos</div>
                              </Label>
                            ...
                            <div className="flex items-center space-x-2 p-2 border rounded-lg hover:bg-gray-50">
                              ...
                              <Label htmlFor={`${area.id}-not-included`} className="flex-1 cursor-pointer">
                                <div className="font-semibold text-xs">No Incluido</div>
                                <div className="text-xs text-gray-500">No se limpiará</div>
                              </Label>
                          ...
                          {currentSelection.selection_type === 'custom' && (
                            <div className="border rounded-lg p-2 space-y-1 max-h-64 overflow-y-auto bg-gray-50">
                              <h5 className="font-semibold text-xs mb-1">Selecciona items:</h5>
                              {areaItems.map(item => {
                                const isSelected = currentSelection.selected_items.some(i => i.item_name === item.item_name);
                                return (
                                  <div key={item.id} className="flex items-start space-x-2 p-1 hover:bg-white rounded">
                                    <Checkbox 
                                      checked={isSelected}
                          ...
                          {currentSelection.selection_type === 'full' && (
                            <div className="border rounded-lg p-2 bg-green-50 max-h-64 overflow-y-auto">
                              <h5 className="font-semibold text-xs mb-1 text-green-800">Todos incluidos:</h5>
                          ...
                          {currentSelection.selection_type === 'not_included' && (
                            <div className="border rounded-lg p-2 bg-red-50">
                              <div className="flex items-center gap-2 text-red-800">
                                <X className="w-3 h-3" />
                                <h5 className="font-semibold text-xs">No se incluirá</h5>
                              </div>
                            </div>
                          )}
                        </div>
